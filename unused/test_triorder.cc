...
main (void)
{
  Remesher::TriangleMeshPtr mesh = Remesher::ModelLoader::load_off_model("/data/offmodels/cad_models/fandisk_rand-flips.off");
  int num_verts = mesh->get_vertices().size();
  int num_faces = mesh->get_faces().size() / 3;
  int cache_size = 24;

  int* out_buffer = new int[num_faces * 3];

  FanVertOptimizeVCacheOnly((int*)&mesh->get_faces()[0], out_buffer, num_verts, num_faces, cache_size);

  for (int i = 0; i < num_faces * 3; ++i)
    mesh->get_faces()[i] = out_buffer[i];

  Remesher::ModelWriter::save_off_model("/data/dev/remesher/debug/optimized_refimpl_24.off", mesh);
